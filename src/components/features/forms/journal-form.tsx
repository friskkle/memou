...
import { createJournal, editJournal, State } from '@/src/lib/actions/journals';
...
/**
 * SharedUser type represents a user that has been added to the journal's collaborator list.
 */
type SharedUser = {
  id: string;
  name: string;
...
/**
 * CreateJournalForm - Component for creating a new journal.
 * Includes functionality to invite co-authors by email.
 *
 * @param uuid - The ID of the current user (creator).
 */
export function CreateJournalForm({ uuid }: { uuid: string }) {
  ...
  // Local state for managing the list of invited co-authors
  ...
  /**
   * Logic to search for a user by email and add them to the sharedUsers list.
   */
  const performEmailSearch = async () => {
    ...
    try {
      ...
      if (user) {
        // Prevent creators from inviting themselves
        if (user.id === uuid) {
  ...
  /**
   * Handles Enter key press in the email input field.
   */
  ...
  /**
   * Removes a user from the sharedUsers list.
   * @param idToRemove - The UUID of the user to remove.
   */
  ...
  return (
    <form action={formAction} style={{ maxWidth: 720, margin: '0 auto' }}>
      {/* Hidden field to submit the array of shared user IDs as a JSON string */}
      <input
        type="hidden"
        name="shared_with"
        value={JSON.stringify(sharedUsers.map((u) => u.id))}
      />

      <div className="rounded-lg bg-white p-4 md:p-6 shadow-md">
        <div className="mb-4">
          {/* Hidden field for creator UUID */}
        ...
        {/* Section for inviting co-authors */}
        <div className="mb-4 border-t border-gray-200 pt-4">
          ...
          <p className="text-xs text-gray-500 mb-2 px-1">
            Type an exact email address and press Enter or click Add to invite.
          </p>
...
/**
 * EditJournalForm - Component for editing an existing journal's details.
 * Pre-populates fields with existing data and handles updates.
 *
 * @param journal - The existing journal object to edit.
 */
export function EditJournalForm({ journal }: { journal: Journal }) {
  const initialState: State = { message: null, errors: {} };
  const [state, formAction] = useActionState(editJournal, initialState);

  // Identity of the journal owner
  const uuid = journal.uuid;

  // Initialize state with existing shared users
  const [sharedUsers, setSharedUsers] = useState<SharedUser[]>(
    journal.shared_with_names,
  );
  const [emailInput, setEmailInput] = useState('');
  const [searchError, setSearchError] = useState<string | null>(null);
  const [isSearching, setIsSearching] = useState(false);
  ...
  return (
    <form action={formAction} style={{ maxWidth: 720, margin: '0 auto' }}>
      {/* Hidden field to submit the updated array of shared user IDs */}
      <input
        type="hidden"
        name="shared_with"
        value={JSON.stringify(sharedUsers.map((u) => u.id))}
      ...
      <div className="rounded-lg bg-white p-4 md:p-6 shadow-md">
        <div className="mb-4">
          {/* Hidden field for the Journal record ID (primary key) */}
          <input type="hidden" name="uuid" value={journal.id} />
        ...
        {/* Section for managing co-authors */}
        <div className="mb-4 border-t border-gray-200 pt-4">
          <label
            className="block text-sm font-medium text-gray-700 mb-1 px-1"
            htmlFor="email-search"
